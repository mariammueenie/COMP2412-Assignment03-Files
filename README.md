...
= lim as n->inf  [ n^2+3n+2/4n^2+2 ] / n^2 
(strat to use frm calc to solve limit)
    
= lim as n->inf   (n^2/n^2)+(3n/n^2)+(2/n^2)/(4n^2/n^2)+(2/4n^2) 
...
(think of what happens to f(n) as n becomes extremely large, and as g(n) becomes extremely large. f(n) will be larger past the point where the 2 functions intersect, therefore the number on the top of the fraction will always be bigger) 
...
Insertion sort basically treats the start of the array as the part that is already sorted.
...
When inserting a value, it is first put at the next available spot at end of heap.

Then compare it to its parent.

If new val is smaller than its parent, swap them and continue going upward until min-heap order is correct.
...
If one of its children is smaller, swap with the smaller child and continue until min-heap order is fixed.

Again this only follows one path through height of heap, therefore extractMin is big O(log n).


### Sorting the 1000 random numbers

I generated 1000 random nums and inserted all of them into MinHeap.
...
For testing I only print first 30 before + after because printing all 1000 makes output kinda ridiculous, but all 1000 vals are still actually generated and sorted.
...
Question 3 uses a hash table with separate chaining.
...
Search its linked list to see if key already exists.

If already exists, update value.

Otherwise make a new node and put into linked list.
...
Then do sequential search through only the linked list at that slot.
...
C++ already uses the word "delete" as a keyword, so function in my code is named removeKey instead.

Find correct slot using hash.

Search linked list.

When matching node found, change the pointers so node is removed from chain, then delete it.
...
This is because hash location depends on m/table capacity.
...
My program also deletes enough values at end to prove the table can shrink again.


## 4

For question 4 I represented each graph using adjacency lists.

The assignment does not say which order to process adjacent verts in when there are multiple choices, so I used alphabetical order consistently.
...
I represented outgoing edges as:
...
The numbers/weights on edges are not needed to decide DFS or BFS traversal order for this question.
